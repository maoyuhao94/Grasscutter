// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: uint32.proto

public final class Uint32 {
  private Uint32() {}
  public static void registerAllExtensions(
      com.google.protobuf.ExtensionRegistryLite registry) {
  }

  public static void registerAllExtensions(
      com.google.protobuf.ExtensionRegistry registry) {
    registerAllExtensions(
        (com.google.protobuf.ExtensionRegistryLite) registry);
  }
  public interface uint32OrBuilder extends
      // @@protoc_insertion_point(interface_extends:uint32)
      com.google.protobuf.MessageOrBuilder {

    /**
     * <code>uint32 minigameId = 14;</code>
     * @return The minigameId.
     */
    int getMinigameId();

    /**
     * <code>bool isOpen = 3;</code>
     * @return The isOpen.
     */
    boolean getIsOpen();

    /**
     * <code>uint32 openTime = 6;</code>
     * @return The openTime.
     */
    int getOpenTime();

    /**
     * <code>.BalloonInfo balloon_info = 13;</code>
     * @return Whether the balloonInfo field is set.
     */
    boolean hasBalloonInfo();
    /**
     * <code>.BalloonInfo balloon_info = 13;</code>
     * @return The balloonInfo.
     */
    BalloonInfoOuterClass.BalloonInfo getBalloonInfo();
    /**
     * <code>.BalloonInfo balloon_info = 13;</code>
     */
    BalloonInfoOuterClass.BalloonInfoOrBuilder getBalloonInfoOrBuilder();

    /**
     * <code>.FallInfo fall_info = 11;</code>
     * @return Whether the fallInfo field is set.
     */
    boolean hasFallInfo();
    /**
     * <code>.FallInfo fall_info = 11;</code>
     * @return The fallInfo.
     */
    FallInfoOuterClass.FallInfo getFallInfo();
    /**
     * <code>.FallInfo fall_info = 11;</code>
     */
    FallInfoOuterClass.FallInfoOrBuilder getFallInfoOrBuilder();

    /**
     * <code>.MusicInfo music_info = 9;</code>
     * @return Whether the musicInfo field is set.
     */
    boolean hasMusicInfo();
    /**
     * <code>.MusicInfo music_info = 9;</code>
     * @return The musicInfo.
     */
    MusicInfoOuterClass.MusicInfo getMusicInfo();
    /**
     * <code>.MusicInfo music_info = 9;</code>
     */
    MusicInfoOuterClass.MusicInfoOrBuilder getMusicInfoOrBuilder();

    public Uint32.uint32.HNMEDAOCKOICase getHNMEDAOCKOICase();
  }
  /**
   * Protobuf type {@code uint32}
   */
  public static final class uint32 extends
      com.google.protobuf.GeneratedMessageV3 implements
      // @@protoc_insertion_point(message_implements:uint32)
      uint32OrBuilder {
  private static final long serialVersionUID = 0L;
    // Use uint32.newBuilder() to construct.
    private uint32(com.google.protobuf.GeneratedMessageV3.Builder<?> builder) {
      super(builder);
    }
    private uint32() {
    }

    @java.lang.Override
    @SuppressWarnings({"unused"})
    protected java.lang.Object newInstance(
        UnusedPrivateParameter unused) {
      return new uint32();
    }

    @java.lang.Override
    public final com.google.protobuf.UnknownFieldSet
    getUnknownFields() {
      return this.unknownFields;
    }
    private uint32(
        com.google.protobuf.CodedInputStream input,
        com.google.protobuf.ExtensionRegistryLite extensionRegistry)
        throws com.google.protobuf.InvalidProtocolBufferException {
      this();
      if (extensionRegistry == null) {
        throw new java.lang.NullPointerException();
      }
      com.google.protobuf.UnknownFieldSet.Builder unknownFields =
          com.google.protobuf.UnknownFieldSet.newBuilder();
      try {
        boolean done = false;
        while (!done) {
          int tag = input.readTag();
          switch (tag) {
            case 0:
              done = true;
              break;
            case 24: {

              isOpen_ = input.readBool();
              break;
            }
            case 48: {

              openTime_ = input.readUInt32();
              break;
            }
            case 74: {
              MusicInfoOuterClass.MusicInfo.Builder subBuilder = null;
              if (hNMEDAOCKOICase_ == 9) {
                subBuilder = ((MusicInfoOuterClass.MusicInfo) hNMEDAOCKOI_).toBuilder();
              }
              hNMEDAOCKOI_ =
                  input.readMessage(MusicInfoOuterClass.MusicInfo.parser(), extensionRegistry);
              if (subBuilder != null) {
                subBuilder.mergeFrom((MusicInfoOuterClass.MusicInfo) hNMEDAOCKOI_);
                hNMEDAOCKOI_ = subBuilder.buildPartial();
              }
              hNMEDAOCKOICase_ = 9;
              break;
            }
            case 90: {
              FallInfoOuterClass.FallInfo.Builder subBuilder = null;
              if (hNMEDAOCKOICase_ == 11) {
                subBuilder = ((FallInfoOuterClass.FallInfo) hNMEDAOCKOI_).toBuilder();
              }
              hNMEDAOCKOI_ =
                  input.readMessage(FallInfoOuterClass.FallInfo.parser(), extensionRegistry);
              if (subBuilder != null) {
                subBuilder.mergeFrom((FallInfoOuterClass.FallInfo) hNMEDAOCKOI_);
                hNMEDAOCKOI_ = subBuilder.buildPartial();
              }
              hNMEDAOCKOICase_ = 11;
              break;
            }
            case 106: {
              BalloonInfoOuterClass.BalloonInfo.Builder subBuilder = null;
              if (hNMEDAOCKOICase_ == 13) {
                subBuilder = ((BalloonInfoOuterClass.BalloonInfo) hNMEDAOCKOI_).toBuilder();
              }
              hNMEDAOCKOI_ =
                  input.readMessage(BalloonInfoOuterClass.BalloonInfo.parser(), extensionRegistry);
              if (subBuilder != null) {
                subBuilder.mergeFrom((BalloonInfoOuterClass.BalloonInfo) hNMEDAOCKOI_);
                hNMEDAOCKOI_ = subBuilder.buildPartial();
              }
              hNMEDAOCKOICase_ = 13;
              break;
            }
            case 112: {

              minigameId_ = input.readUInt32();
              break;
            }
            default: {
              if (!parseUnknownField(
                  input, unknownFields, extensionRegistry, tag)) {
                done = true;
              }
              break;
            }
          }
        }
      } catch (com.google.protobuf.InvalidProtocolBufferException e) {
        throw e.setUnfinishedMessage(this);
      } catch (java.io.IOException e) {
        throw new com.google.protobuf.InvalidProtocolBufferException(
            e).setUnfinishedMessage(this);
      } finally {
        this.unknownFields = unknownFields.build();
        makeExtensionsImmutable();
      }
    }
    public static final com.google.protobuf.Descriptors.Descriptor
        getDescriptor() {
      return Uint32.internal_static_uint32_descriptor;
    }

    @java.lang.Override
    protected com.google.protobuf.GeneratedMessageV3.FieldAccessorTable
        internalGetFieldAccessorTable() {
      return Uint32.internal_static_uint32_fieldAccessorTable
          .ensureFieldAccessorsInitialized(
              Uint32.uint32.class, Uint32.uint32.Builder.class);
    }

    private int hNMEDAOCKOICase_ = 0;
    private java.lang.Object hNMEDAOCKOI_;
    public enum HNMEDAOCKOICase
        implements com.google.protobuf.Internal.EnumLite,
            com.google.protobuf.AbstractMessage.InternalOneOfEnum {
      BALLOON_INFO(13),
      FALL_INFO(11),
      MUSIC_INFO(9),
      HNMEDAOCKOI_NOT_SET(0);
      private final int value;
      private HNMEDAOCKOICase(int value) {
        this.value = value;
      }
      /**
       * @param value The number of the enum to look for.
       * @return The enum associated with the given number.
       * @deprecated Use {@link #forNumber(int)} instead.
       */
      @java.lang.Deprecated
      public static HNMEDAOCKOICase valueOf(int value) {
        return forNumber(value);
      }

      public static HNMEDAOCKOICase forNumber(int value) {
        switch (value) {
          case 13: return BALLOON_INFO;
          case 11: return FALL_INFO;
          case 9: return MUSIC_INFO;
          case 0: return HNMEDAOCKOI_NOT_SET;
          default: return null;
        }
      }
      public int getNumber() {
        return this.value;
      }
    };

    public HNMEDAOCKOICase
    getHNMEDAOCKOICase() {
      return HNMEDAOCKOICase.forNumber(
          hNMEDAOCKOICase_);
    }

    public static final int MINIGAMEID_FIELD_NUMBER = 14;
    private int minigameId_;
    /**
     * <code>uint32 minigameId = 14;</code>
     * @return The minigameId.
     */
    @java.lang.Override
    public int getMinigameId() {
      return minigameId_;
    }

    public static final int ISOPEN_FIELD_NUMBER = 3;
    private boolean isOpen_;
    /**
     * <code>bool isOpen = 3;</code>
     * @return The isOpen.
     */
    @java.lang.Override
    public boolean getIsOpen() {
      return isOpen_;
    }

    public static final int OPENTIME_FIELD_NUMBER = 6;
    private int openTime_;
    /**
     * <code>uint32 openTime = 6;</code>
     * @return The openTime.
     */
    @java.lang.Override
    public int getOpenTime() {
      return openTime_;
    }

    public static final int BALLOON_INFO_FIELD_NUMBER = 13;
    /**
     * <code>.BalloonInfo balloon_info = 13;</code>
     * @return Whether the balloonInfo field is set.
     */
    @java.lang.Override
    public boolean hasBalloonInfo() {
      return hNMEDAOCKOICase_ == 13;
    }
    /**
     * <code>.BalloonInfo balloon_info = 13;</code>
     * @return The balloonInfo.
     */
    @java.lang.Override
    public BalloonInfoOuterClass.BalloonInfo getBalloonInfo() {
      if (hNMEDAOCKOICase_ == 13) {
         return (BalloonInfoOuterClass.BalloonInfo) hNMEDAOCKOI_;
      }
      return BalloonInfoOuterClass.BalloonInfo.getDefaultInstance();
    }
    /**
     * <code>.BalloonInfo balloon_info = 13;</code>
     */
    @java.lang.Override
    public BalloonInfoOuterClass.BalloonInfoOrBuilder getBalloonInfoOrBuilder() {
      if (hNMEDAOCKOICase_ == 13) {
         return (BalloonInfoOuterClass.BalloonInfo) hNMEDAOCKOI_;
      }
      return BalloonInfoOuterClass.BalloonInfo.getDefaultInstance();
    }

    public static final int FALL_INFO_FIELD_NUMBER = 11;
    /**
     * <code>.FallInfo fall_info = 11;</code>
     * @return Whether the fallInfo field is set.
     */
    @java.lang.Override
    public boolean hasFallInfo() {
      return hNMEDAOCKOICase_ == 11;
    }
    /**
     * <code>.FallInfo fall_info = 11;</code>
     * @return The fallInfo.
     */
    @java.lang.Override
    public FallInfoOuterClass.FallInfo getFallInfo() {
      if (hNMEDAOCKOICase_ == 11) {
         return (FallInfoOuterClass.FallInfo) hNMEDAOCKOI_;
      }
      return FallInfoOuterClass.FallInfo.getDefaultInstance();
    }
    /**
     * <code>.FallInfo fall_info = 11;</code>
     */
    @java.lang.Override
    public FallInfoOuterClass.FallInfoOrBuilder getFallInfoOrBuilder() {
      if (hNMEDAOCKOICase_ == 11) {
         return (FallInfoOuterClass.FallInfo) hNMEDAOCKOI_;
      }
      return FallInfoOuterClass.FallInfo.getDefaultInstance();
    }

    public static final int MUSIC_INFO_FIELD_NUMBER = 9;
    /**
     * <code>.MusicInfo music_info = 9;</code>
     * @return Whether the musicInfo field is set.
     */
    @java.lang.Override
    public boolean hasMusicInfo() {
      return hNMEDAOCKOICase_ == 9;
    }
    /**
     * <code>.MusicInfo music_info = 9;</code>
     * @return The musicInfo.
     */
    @java.lang.Override
    public MusicInfoOuterClass.MusicInfo getMusicInfo() {
      if (hNMEDAOCKOICase_ == 9) {
         return (MusicInfoOuterClass.MusicInfo) hNMEDAOCKOI_;
      }
      return MusicInfoOuterClass.MusicInfo.getDefaultInstance();
    }
    /**
     * <code>.MusicInfo music_info = 9;</code>
     */
    @java.lang.Override
    public MusicInfoOuterClass.MusicInfoOrBuilder getMusicInfoOrBuilder() {
      if (hNMEDAOCKOICase_ == 9) {
         return (MusicInfoOuterClass.MusicInfo) hNMEDAOCKOI_;
      }
      return MusicInfoOuterClass.MusicInfo.getDefaultInstance();
    }

    private byte memoizedIsInitialized = -1;
    @java.lang.Override
    public final boolean isInitialized() {
      byte isInitialized = memoizedIsInitialized;
      if (isInitialized == 1) return true;
      if (isInitialized == 0) return false;

      memoizedIsInitialized = 1;
      return true;
    }

    @java.lang.Override
    public void writeTo(com.google.protobuf.CodedOutputStream output)
                        throws java.io.IOException {
      if (isOpen_ != false) {
        output.writeBool(3, isOpen_);
      }
      if (openTime_ != 0) {
        output.writeUInt32(6, openTime_);
      }
      if (hNMEDAOCKOICase_ == 9) {
        output.writeMessage(9, (MusicInfoOuterClass.MusicInfo) hNMEDAOCKOI_);
      }
      if (hNMEDAOCKOICase_ == 11) {
        output.writeMessage(11, (FallInfoOuterClass.FallInfo) hNMEDAOCKOI_);
      }
      if (hNMEDAOCKOICase_ == 13) {
        output.writeMessage(13, (BalloonInfoOuterClass.BalloonInfo) hNMEDAOCKOI_);
      }
      if (minigameId_ != 0) {
        output.writeUInt32(14, minigameId_);
      }
      unknownFields.writeTo(output);
    }

    @java.lang.Override
    public int getSerializedSize() {
      int size = memoizedSize;
      if (size != -1) return size;

      size = 0;
      if (isOpen_ != false) {
        size += com.google.protobuf.CodedOutputStream
          .computeBoolSize(3, isOpen_);
      }
      if (openTime_ != 0) {
        size += com.google.protobuf.CodedOutputStream
          .computeUInt32Size(6, openTime_);
      }
      if (hNMEDAOCKOICase_ == 9) {
        size += com.google.protobuf.CodedOutputStream
          .computeMessageSize(9, (MusicInfoOuterClass.MusicInfo) hNMEDAOCKOI_);
      }
      if (hNMEDAOCKOICase_ == 11) {
        size += com.google.protobuf.CodedOutputStream
          .computeMessageSize(11, (FallInfoOuterClass.FallInfo) hNMEDAOCKOI_);
      }
      if (hNMEDAOCKOICase_ == 13) {
        size += com.google.protobuf.CodedOutputStream
          .computeMessageSize(13, (BalloonInfoOuterClass.BalloonInfo) hNMEDAOCKOI_);
      }
      if (minigameId_ != 0) {
        size += com.google.protobuf.CodedOutputStream
          .computeUInt32Size(14, minigameId_);
      }
      size += unknownFields.getSerializedSize();
      memoizedSize = size;
      return size;
    }

    @java.lang.Override
    public boolean equals(final java.lang.Object obj) {
      if (obj == this) {
       return true;
      }
      if (!(obj instanceof Uint32.uint32)) {
        return super.equals(obj);
      }
      Uint32.uint32 other = (Uint32.uint32) obj;

      if (getMinigameId()
          != other.getMinigameId()) return false;
      if (getIsOpen()
          != other.getIsOpen()) return false;
      if (getOpenTime()
          != other.getOpenTime()) return false;
      if (!getHNMEDAOCKOICase().equals(other.getHNMEDAOCKOICase())) return false;
      switch (hNMEDAOCKOICase_) {
        case 13:
          if (!getBalloonInfo()
              .equals(other.getBalloonInfo())) return false;
          break;
        case 11:
          if (!getFallInfo()
              .equals(other.getFallInfo())) return false;
          break;
        case 9:
          if (!getMusicInfo()
              .equals(other.getMusicInfo())) return false;
          break;
        case 0:
        default:
      }
      if (!unknownFields.equals(other.unknownFields)) return false;
      return true;
    }

    @java.lang.Override
    public int hashCode() {
      if (memoizedHashCode != 0) {
        return memoizedHashCode;
      }
      int hash = 41;
      hash = (19 * hash) + getDescriptor().hashCode();
      hash = (37 * hash) + MINIGAMEID_FIELD_NUMBER;
      hash = (53 * hash) + getMinigameId();
      hash = (37 * hash) + ISOPEN_FIELD_NUMBER;
      hash = (53 * hash) + com.google.protobuf.Internal.hashBoolean(
          getIsOpen());
      hash = (37 * hash) + OPENTIME_FIELD_NUMBER;
      hash = (53 * hash) + getOpenTime();
      switch (hNMEDAOCKOICase_) {
        case 13:
          hash = (37 * hash) + BALLOON_INFO_FIELD_NUMBER;
          hash = (53 * hash) + getBalloonInfo().hashCode();
          break;
        case 11:
          hash = (37 * hash) + FALL_INFO_FIELD_NUMBER;
          hash = (53 * hash) + getFallInfo().hashCode();
          break;
        case 9:
          hash = (37 * hash) + MUSIC_INFO_FIELD_NUMBER;
          hash = (53 * hash) + getMusicInfo().hashCode();
          break;
        case 0:
        default:
      }
      hash = (29 * hash) + unknownFields.hashCode();
      memoizedHashCode = hash;
      return hash;
    }

    public static Uint32.uint32 parseFrom(
        java.nio.ByteBuffer data)
        throws com.google.protobuf.InvalidProtocolBufferException {
      return PARSER.parseFrom(data);
    }
    public static Uint32.uint32 parseFrom(
        java.nio.ByteBuffer data,
        com.google.protobuf.ExtensionRegistryLite extensionRegistry)
        throws com.google.protobuf.InvalidProtocolBufferException {
      return PARSER.parseFrom(data, extensionRegistry);
    }
    public static Uint32.uint32 parseFrom(
        com.google.protobuf.ByteString data)
        throws com.google.protobuf.InvalidProtocolBufferException {
      return PARSER.parseFrom(data);
    }
    public static Uint32.uint32 parseFrom(
        com.google.protobuf.ByteString data,
        com.google.protobuf.ExtensionRegistryLite extensionRegistry)
        throws com.google.protobuf.InvalidProtocolBufferException {
      return PARSER.parseFrom(data, extensionRegistry);
    }
    public static Uint32.uint32 parseFrom(byte[] data)
        throws com.google.protobuf.InvalidProtocolBufferException {
      return PARSER.parseFrom(data);
    }
    public static Uint32.uint32 parseFrom(
        byte[] data,
        com.google.protobuf.ExtensionRegistryLite extensionRegistry)
        throws com.google.protobuf.InvalidProtocolBufferException {
      return PARSER.parseFrom(data, extensionRegistry);
    }
    public static Uint32.uint32 parseFrom(java.io.InputStream input)
        throws java.io.IOException {
      return com.google.protobuf.GeneratedMessageV3
          .parseWithIOException(PARSER, input);
    }
    public static Uint32.uint32 parseFrom(
        java.io.InputStream input,
        com.google.protobuf.ExtensionRegistryLite extensionRegistry)
        throws java.io.IOException {
      return com.google.protobuf.GeneratedMessageV3
          .parseWithIOException(PARSER, input, extensionRegistry);
    }
    public static Uint32.uint32 parseDelimitedFrom(java.io.InputStream input)
        throws java.io.IOException {
      return com.google.protobuf.GeneratedMessageV3
          .parseDelimitedWithIOException(PARSER, input);
    }
    public static Uint32.uint32 parseDelimitedFrom(
        java.io.InputStream input,
        com.google.protobuf.ExtensionRegistryLite extensionRegistry)
        throws java.io.IOException {
      return com.google.protobuf.GeneratedMessageV3
          .parseDelimitedWithIOException(PARSER, input, extensionRegistry);
    }
    public static Uint32.uint32 parseFrom(
        com.google.protobuf.CodedInputStream input)
        throws java.io.IOException {
      return com.google.protobuf.GeneratedMessageV3
          .parseWithIOException(PARSER, input);
    }
    public static Uint32.uint32 parseFrom(
        com.google.protobuf.CodedInputStream input,
        com.google.protobuf.ExtensionRegistryLite extensionRegistry)
        throws java.io.IOException {
      return com.google.protobuf.GeneratedMessageV3
          .parseWithIOException(PARSER, input, extensionRegistry);
    }

    @java.lang.Override
    public Builder newBuilderForType() { return newBuilder(); }
    public static Builder newBuilder() {
      return DEFAULT_INSTANCE.toBuilder();
    }
    public static Builder newBuilder(Uint32.uint32 prototype) {
      return DEFAULT_INSTANCE.toBuilder().mergeFrom(prototype);
    }
    @java.lang.Override
    public Builder toBuilder() {
      return this == DEFAULT_INSTANCE
          ? new Builder() : new Builder().mergeFrom(this);
    }

    @java.lang.Override
    protected Builder newBuilderForType(
        com.google.protobuf.GeneratedMessageV3.BuilderParent parent) {
      Builder builder = new Builder(parent);
      return builder;
    }
    /**
     * Protobuf type {@code uint32}
     */
    public static final class Builder extends
        com.google.protobuf.GeneratedMessageV3.Builder<Builder> implements
        // @@protoc_insertion_point(builder_implements:uint32)
        Uint32.uint32OrBuilder {
      public static final com.google.protobuf.Descriptors.Descriptor
          getDescriptor() {
        return Uint32.internal_static_uint32_descriptor;
      }

      @java.lang.Override
      protected com.google.protobuf.GeneratedMessageV3.FieldAccessorTable
          internalGetFieldAccessorTable() {
        return Uint32.internal_static_uint32_fieldAccessorTable
            .ensureFieldAccessorsInitialized(
                Uint32.uint32.class, Uint32.uint32.Builder.class);
      }

      // Construct using Uint32.uint32.newBuilder()
      private Builder() {
        maybeForceBuilderInitialization();
      }

      private Builder(
          com.google.protobuf.GeneratedMessageV3.BuilderParent parent) {
        super(parent);
        maybeForceBuilderInitialization();
      }
      private void maybeForceBuilderInitialization() {
        if (com.google.protobuf.GeneratedMessageV3
                .alwaysUseFieldBuilders) {
        }
      }
      @java.lang.Override
      public Builder clear() {
        super.clear();
        minigameId_ = 0;

        isOpen_ = false;

        openTime_ = 0;

        hNMEDAOCKOICase_ = 0;
        hNMEDAOCKOI_ = null;
        return this;
      }

      @java.lang.Override
      public com.google.protobuf.Descriptors.Descriptor
          getDescriptorForType() {
        return Uint32.internal_static_uint32_descriptor;
      }

      @java.lang.Override
      public Uint32.uint32 getDefaultInstanceForType() {
        return Uint32.uint32.getDefaultInstance();
      }

      @java.lang.Override
      public Uint32.uint32 build() {
        Uint32.uint32 result = buildPartial();
        if (!result.isInitialized()) {
          throw newUninitializedMessageException(result);
        }
        return result;
      }

      @java.lang.Override
      public Uint32.uint32 buildPartial() {
        Uint32.uint32 result = new Uint32.uint32(this);
        result.minigameId_ = minigameId_;
        result.isOpen_ = isOpen_;
        result.openTime_ = openTime_;
        if (hNMEDAOCKOICase_ == 13) {
          if (balloonInfoBuilder_ == null) {
            result.hNMEDAOCKOI_ = hNMEDAOCKOI_;
          } else {
            result.hNMEDAOCKOI_ = balloonInfoBuilder_.build();
          }
        }
        if (hNMEDAOCKOICase_ == 11) {
          if (fallInfoBuilder_ == null) {
            result.hNMEDAOCKOI_ = hNMEDAOCKOI_;
          } else {
            result.hNMEDAOCKOI_ = fallInfoBuilder_.build();
          }
        }
        if (hNMEDAOCKOICase_ == 9) {
          if (musicInfoBuilder_ == null) {
            result.hNMEDAOCKOI_ = hNMEDAOCKOI_;
          } else {
            result.hNMEDAOCKOI_ = musicInfoBuilder_.build();
          }
        }
        result.hNMEDAOCKOICase_ = hNMEDAOCKOICase_;
        onBuilt();
        return result;
      }

      @java.lang.Override
      public Builder clone() {
        return super.clone();
      }
      @java.lang.Override
      public Builder setField(
          com.google.protobuf.Descriptors.FieldDescriptor field,
          java.lang.Object value) {
        return super.setField(field, value);
      }
      @java.lang.Override
      public Builder clearField(
          com.google.protobuf.Descriptors.FieldDescriptor field) {
        return super.clearField(field);
      }
      @java.lang.Override
      public Builder clearOneof(
          com.google.protobuf.Descriptors.OneofDescriptor oneof) {
        return super.clearOneof(oneof);
      }
      @java.lang.Override
      public Builder setRepeatedField(
          com.google.protobuf.Descriptors.FieldDescriptor field,
          int index, java.lang.Object value) {
        return super.setRepeatedField(field, index, value);
      }
      @java.lang.Override
      public Builder addRepeatedField(
          com.google.protobuf.Descriptors.FieldDescriptor field,
          java.lang.Object value) {
        return super.addRepeatedField(field, value);
      }
      @java.lang.Override
      public Builder mergeFrom(com.google.protobuf.Message other) {
        if (other instanceof Uint32.uint32) {
          return mergeFrom((Uint32.uint32)other);
        } else {
          super.mergeFrom(other);
          return this;
        }
      }

      public Builder mergeFrom(Uint32.uint32 other) {
        if (other == Uint32.uint32.getDefaultInstance()) return this;
        if (other.getMinigameId() != 0) {
          setMinigameId(other.getMinigameId());
        }
        if (other.getIsOpen() != false) {
          setIsOpen(other.getIsOpen());
        }
        if (other.getOpenTime() != 0) {
          setOpenTime(other.getOpenTime());
        }
        switch (other.getHNMEDAOCKOICase()) {
          case BALLOON_INFO: {
            mergeBalloonInfo(other.getBalloonInfo());
            break;
          }
          case FALL_INFO: {
            mergeFallInfo(other.getFallInfo());
            break;
          }
          case MUSIC_INFO: {
            mergeMusicInfo(other.getMusicInfo());
            break;
          }
          case HNMEDAOCKOI_NOT_SET: {
            break;
          }
        }
        this.mergeUnknownFields(other.unknownFields);
        onChanged();
        return this;
      }

      @java.lang.Override
      public final boolean isInitialized() {
        return true;
      }

      @java.lang.Override
      public Builder mergeFrom(
          com.google.protobuf.CodedInputStream input,
          com.google.protobuf.ExtensionRegistryLite extensionRegistry)
          throws java.io.IOException {
        Uint32.uint32 parsedMessage = null;
        try {
          parsedMessage = PARSER.parsePartialFrom(input, extensionRegistry);
        } catch (com.google.protobuf.InvalidProtocolBufferException e) {
          parsedMessage = (Uint32.uint32) e.getUnfinishedMessage();
          throw e.unwrapIOException();
        } finally {
          if (parsedMessage != null) {
            mergeFrom(parsedMessage);
          }
        }
        return this;
      }
      private int hNMEDAOCKOICase_ = 0;
      private java.lang.Object hNMEDAOCKOI_;
      public HNMEDAOCKOICase
          getHNMEDAOCKOICase() {
        return HNMEDAOCKOICase.forNumber(
            hNMEDAOCKOICase_);
      }

      public Builder clearHNMEDAOCKOI() {
        hNMEDAOCKOICase_ = 0;
        hNMEDAOCKOI_ = null;
        onChanged();
        return this;
      }


      private int minigameId_ ;
      /**
       * <code>uint32 minigameId = 14;</code>
       * @return The minigameId.
       */
      @java.lang.Override
      public int getMinigameId() {
        return minigameId_;
      }
      /**
       * <code>uint32 minigameId = 14;</code>
       * @param value The minigameId to set.
       * @return This builder for chaining.
       */
      public Builder setMinigameId(int value) {
        
        minigameId_ = value;
        onChanged();
        return this;
      }
      /**
       * <code>uint32 minigameId = 14;</code>
       * @return This builder for chaining.
       */
      public Builder clearMinigameId() {
        
        minigameId_ = 0;
        onChanged();
        return this;
      }

      private boolean isOpen_ ;
      /**
       * <code>bool isOpen = 3;</code>
       * @return The isOpen.
       */
      @java.lang.Override
      public boolean getIsOpen() {
        return isOpen_;
      }
      /**
       * <code>bool isOpen = 3;</code>
       * @param value The isOpen to set.
       * @return This builder for chaining.
       */
      public Builder setIsOpen(boolean value) {
        
        isOpen_ = value;
        onChanged();
        return this;
      }
      /**
       * <code>bool isOpen = 3;</code>
       * @return This builder for chaining.
       */
      public Builder clearIsOpen() {
        
        isOpen_ = false;
        onChanged();
        return this;
      }

      private int openTime_ ;
      /**
       * <code>uint32 openTime = 6;</code>
       * @return The openTime.
       */
      @java.lang.Override
      public int getOpenTime() {
        return openTime_;
      }
      /**
       * <code>uint32 openTime = 6;</code>
       * @param value The openTime to set.
       * @return This builder for chaining.
       */
      public Builder setOpenTime(int value) {
        
        openTime_ = value;
        onChanged();
        return this;
      }
      /**
       * <code>uint32 openTime = 6;</code>
       * @return This builder for chaining.
       */
      public Builder clearOpenTime() {
        
        openTime_ = 0;
        onChanged();
        return this;
      }

      private com.google.protobuf.SingleFieldBuilderV3<
          BalloonInfoOuterClass.BalloonInfo, BalloonInfoOuterClass.BalloonInfo.Builder, BalloonInfoOuterClass.BalloonInfoOrBuilder> balloonInfoBuilder_;
      /**
       * <code>.BalloonInfo balloon_info = 13;</code>
       * @return Whether the balloonInfo field is set.
       */
      @java.lang.Override
      public boolean hasBalloonInfo() {
        return hNMEDAOCKOICase_ == 13;
      }
      /**
       * <code>.BalloonInfo balloon_info = 13;</code>
       * @return The balloonInfo.
       */
      @java.lang.Override
      public BalloonInfoOuterClass.BalloonInfo getBalloonInfo() {
        if (balloonInfoBuilder_ == null) {
          if (hNMEDAOCKOICase_ == 13) {
            return (BalloonInfoOuterClass.BalloonInfo) hNMEDAOCKOI_;
          }
          return BalloonInfoOuterClass.BalloonInfo.getDefaultInstance();
        } else {
          if (hNMEDAOCKOICase_ == 13) {
            return balloonInfoBuilder_.getMessage();
          }
          return BalloonInfoOuterClass.BalloonInfo.getDefaultInstance();
        }
      }
      /**
       * <code>.BalloonInfo balloon_info = 13;</code>
       */
      public Builder setBalloonInfo(BalloonInfoOuterClass.BalloonInfo value) {
        if (balloonInfoBuilder_ == null) {
          if (value == null) {
            throw new NullPointerException();
          }
          hNMEDAOCKOI_ = value;
          onChanged();
        } else {
          balloonInfoBuilder_.setMessage(value);
        }
        hNMEDAOCKOICase_ = 13;
        return this;
      }
      /**
       * <code>.BalloonInfo balloon_info = 13;</code>
       */
      public Builder setBalloonInfo(
          BalloonInfoOuterClass.BalloonInfo.Builder builderForValue) {
        if (balloonInfoBuilder_ == null) {
          hNMEDAOCKOI_ = builderForValue.build();
          onChanged();
        } else {
          balloonInfoBuilder_.setMessage(builderForValue.build());
        }
        hNMEDAOCKOICase_ = 13;
        return this;
      }
      /**
       * <code>.BalloonInfo balloon_info = 13;</code>
       */
      public Builder mergeBalloonInfo(BalloonInfoOuterClass.BalloonInfo value) {
        if (balloonInfoBuilder_ == null) {
          if (hNMEDAOCKOICase_ == 13 &&
              hNMEDAOCKOI_ != BalloonInfoOuterClass.BalloonInfo.getDefaultInstance()) {
            hNMEDAOCKOI_ = BalloonInfoOuterClass.BalloonInfo.newBuilder((BalloonInfoOuterClass.BalloonInfo) hNMEDAOCKOI_)
                .mergeFrom(value).buildPartial();
          } else {
            hNMEDAOCKOI_ = value;
          }
          onChanged();
        } else {
          if (hNMEDAOCKOICase_ == 13) {
            balloonInfoBuilder_.mergeFrom(value);
          }
          balloonInfoBuilder_.setMessage(value);
        }
        hNMEDAOCKOICase_ = 13;
        return this;
      }
      /**
       * <code>.BalloonInfo balloon_info = 13;</code>
       */
      public Builder clearBalloonInfo() {
        if (balloonInfoBuilder_ == null) {
          if (hNMEDAOCKOICase_ == 13) {
            hNMEDAOCKOICase_ = 0;
            hNMEDAOCKOI_ = null;
            onChanged();
          }
        } else {
          if (hNMEDAOCKOICase_ == 13) {
            hNMEDAOCKOICase_ = 0;
            hNMEDAOCKOI_ = null;
          }
          balloonInfoBuilder_.clear();
        }
        return this;
      }
      /**
       * <code>.BalloonInfo balloon_info = 13;</code>
       */
      public BalloonInfoOuterClass.BalloonInfo.Builder getBalloonInfoBuilder() {
        return getBalloonInfoFieldBuilder().getBuilder();
      }
      /**
       * <code>.BalloonInfo balloon_info = 13;</code>
       */
      @java.lang.Override
      public BalloonInfoOuterClass.BalloonInfoOrBuilder getBalloonInfoOrBuilder() {
        if ((hNMEDAOCKOICase_ == 13) && (balloonInfoBuilder_ != null)) {
          return balloonInfoBuilder_.getMessageOrBuilder();
        } else {
          if (hNMEDAOCKOICase_ == 13) {
            return (BalloonInfoOuterClass.BalloonInfo) hNMEDAOCKOI_;
          }
          return BalloonInfoOuterClass.BalloonInfo.getDefaultInstance();
        }
      }
      /**
       * <code>.BalloonInfo balloon_info = 13;</code>
       */
      private com.google.protobuf.SingleFieldBuilderV3<
          BalloonInfoOuterClass.BalloonInfo, BalloonInfoOuterClass.BalloonInfo.Builder, BalloonInfoOuterClass.BalloonInfoOrBuilder> 
          getBalloonInfoFieldBuilder() {
        if (balloonInfoBuilder_ == null) {
          if (!(hNMEDAOCKOICase_ == 13)) {
            hNMEDAOCKOI_ = BalloonInfoOuterClass.BalloonInfo.getDefaultInstance();
          }
          balloonInfoBuilder_ = new com.google.protobuf.SingleFieldBuilderV3<
              BalloonInfoOuterClass.BalloonInfo, BalloonInfoOuterClass.BalloonInfo.Builder, BalloonInfoOuterClass.BalloonInfoOrBuilder>(
                  (BalloonInfoOuterClass.BalloonInfo) hNMEDAOCKOI_,
                  getParentForChildren(),
                  isClean());
          hNMEDAOCKOI_ = null;
        }
        hNMEDAOCKOICase_ = 13;
        onChanged();;
        return balloonInfoBuilder_;
      }

      private com.google.protobuf.SingleFieldBuilderV3<
          FallInfoOuterClass.FallInfo, FallInfoOuterClass.FallInfo.Builder, FallInfoOuterClass.FallInfoOrBuilder> fallInfoBuilder_;
      /**
       * <code>.FallInfo fall_info = 11;</code>
       * @return Whether the fallInfo field is set.
       */
      @java.lang.Override
      public boolean hasFallInfo() {
        return hNMEDAOCKOICase_ == 11;
      }
      /**
       * <code>.FallInfo fall_info = 11;</code>
       * @return The fallInfo.
       */
      @java.lang.Override
      public FallInfoOuterClass.FallInfo getFallInfo() {
        if (fallInfoBuilder_ == null) {
          if (hNMEDAOCKOICase_ == 11) {
            return (FallInfoOuterClass.FallInfo) hNMEDAOCKOI_;
          }
          return FallInfoOuterClass.FallInfo.getDefaultInstance();
        } else {
          if (hNMEDAOCKOICase_ == 11) {
            return fallInfoBuilder_.getMessage();
          }
          return FallInfoOuterClass.FallInfo.getDefaultInstance();
        }
      }
      /**
       * <code>.FallInfo fall_info = 11;</code>
       */
      public Builder setFallInfo(FallInfoOuterClass.FallInfo value) {
        if (fallInfoBuilder_ == null) {
          if (value == null) {
            throw new NullPointerException();
          }
          hNMEDAOCKOI_ = value;
          onChanged();
        } else {
          fallInfoBuilder_.setMessage(value);
        }
        hNMEDAOCKOICase_ = 11;
        return this;
      }
      /**
       * <code>.FallInfo fall_info = 11;</code>
       */
      public Builder setFallInfo(
          FallInfoOuterClass.FallInfo.Builder builderForValue) {
        if (fallInfoBuilder_ == null) {
          hNMEDAOCKOI_ = builderForValue.build();
          onChanged();
        } else {
          fallInfoBuilder_.setMessage(builderForValue.build());
        }
        hNMEDAOCKOICase_ = 11;
        return this;
      }
      /**
       * <code>.FallInfo fall_info = 11;</code>
       */
      public Builder mergeFallInfo(FallInfoOuterClass.FallInfo value) {
        if (fallInfoBuilder_ == null) {
          if (hNMEDAOCKOICase_ == 11 &&
              hNMEDAOCKOI_ != FallInfoOuterClass.FallInfo.getDefaultInstance()) {
            hNMEDAOCKOI_ = FallInfoOuterClass.FallInfo.newBuilder((FallInfoOuterClass.FallInfo) hNMEDAOCKOI_)
                .mergeFrom(value).buildPartial();
          } else {
            hNMEDAOCKOI_ = value;
          }
          onChanged();
        } else {
          if (hNMEDAOCKOICase_ == 11) {
            fallInfoBuilder_.mergeFrom(value);
          }
          fallInfoBuilder_.setMessage(value);
        }
        hNMEDAOCKOICase_ = 11;
        return this;
      }
      /**
       * <code>.FallInfo fall_info = 11;</code>
       */
      public Builder clearFallInfo() {
        if (fallInfoBuilder_ == null) {
          if (hNMEDAOCKOICase_ == 11) {
            hNMEDAOCKOICase_ = 0;
            hNMEDAOCKOI_ = null;
            onChanged();
          }
        } else {
          if (hNMEDAOCKOICase_ == 11) {
            hNMEDAOCKOICase_ = 0;
            hNMEDAOCKOI_ = null;
          }
          fallInfoBuilder_.clear();
        }
        return this;
      }
      /**
       * <code>.FallInfo fall_info = 11;</code>
       */
      public FallInfoOuterClass.FallInfo.Builder getFallInfoBuilder() {
        return getFallInfoFieldBuilder().getBuilder();
      }
      /**
       * <code>.FallInfo fall_info = 11;</code>
       */
      @java.lang.Override
      public FallInfoOuterClass.FallInfoOrBuilder getFallInfoOrBuilder() {
        if ((hNMEDAOCKOICase_ == 11) && (fallInfoBuilder_ != null)) {
          return fallInfoBuilder_.getMessageOrBuilder();
        } else {
          if (hNMEDAOCKOICase_ == 11) {
            return (FallInfoOuterClass.FallInfo) hNMEDAOCKOI_;
          }
          return FallInfoOuterClass.FallInfo.getDefaultInstance();
        }
      }
      /**
       * <code>.FallInfo fall_info = 11;</code>
       */
      private com.google.protobuf.SingleFieldBuilderV3<
          FallInfoOuterClass.FallInfo, FallInfoOuterClass.FallInfo.Builder, FallInfoOuterClass.FallInfoOrBuilder> 
          getFallInfoFieldBuilder() {
        if (fallInfoBuilder_ == null) {
          if (!(hNMEDAOCKOICase_ == 11)) {
            hNMEDAOCKOI_ = FallInfoOuterClass.FallInfo.getDefaultInstance();
          }
          fallInfoBuilder_ = new com.google.protobuf.SingleFieldBuilderV3<
              FallInfoOuterClass.FallInfo, FallInfoOuterClass.FallInfo.Builder, FallInfoOuterClass.FallInfoOrBuilder>(
                  (FallInfoOuterClass.FallInfo) hNMEDAOCKOI_,
                  getParentForChildren(),
                  isClean());
          hNMEDAOCKOI_ = null;
        }
        hNMEDAOCKOICase_ = 11;
        onChanged();;
        return fallInfoBuilder_;
      }

      private com.google.protobuf.SingleFieldBuilderV3<
          MusicInfoOuterClass.MusicInfo, MusicInfoOuterClass.MusicInfo.Builder, MusicInfoOuterClass.MusicInfoOrBuilder> musicInfoBuilder_;
      /**
       * <code>.MusicInfo music_info = 9;</code>
       * @return Whether the musicInfo field is set.
       */
      @java.lang.Override
      public boolean hasMusicInfo() {
        return hNMEDAOCKOICase_ == 9;
      }
      /**
       * <code>.MusicInfo music_info = 9;</code>
       * @return The musicInfo.
       */
      @java.lang.Override
      public MusicInfoOuterClass.MusicInfo getMusicInfo() {
        if (musicInfoBuilder_ == null) {
          if (hNMEDAOCKOICase_ == 9) {
            return (MusicInfoOuterClass.MusicInfo) hNMEDAOCKOI_;
          }
          return MusicInfoOuterClass.MusicInfo.getDefaultInstance();
        } else {
          if (hNMEDAOCKOICase_ == 9) {
            return musicInfoBuilder_.getMessage();
          }
          return MusicInfoOuterClass.MusicInfo.getDefaultInstance();
        }
      }
      /**
       * <code>.MusicInfo music_info = 9;</code>
       */
      public Builder setMusicInfo(MusicInfoOuterClass.MusicInfo value) {
        if (musicInfoBuilder_ == null) {
          if (value == null) {
            throw new NullPointerException();
          }
          hNMEDAOCKOI_ = value;
          onChanged();
        } else {
          musicInfoBuilder_.setMessage(value);
        }
        hNMEDAOCKOICase_ = 9;
        return this;
      }
      /**
       * <code>.MusicInfo music_info = 9;</code>
       */
      public Builder setMusicInfo(
          MusicInfoOuterClass.MusicInfo.Builder builderForValue) {
        if (musicInfoBuilder_ == null) {
          hNMEDAOCKOI_ = builderForValue.build();
          onChanged();
        } else {
          musicInfoBuilder_.setMessage(builderForValue.build());
        }
        hNMEDAOCKOICase_ = 9;
        return this;
      }
      /**
       * <code>.MusicInfo music_info = 9;</code>
       */
      public Builder mergeMusicInfo(MusicInfoOuterClass.MusicInfo value) {
        if (musicInfoBuilder_ == null) {
          if (hNMEDAOCKOICase_ == 9 &&
              hNMEDAOCKOI_ != MusicInfoOuterClass.MusicInfo.getDefaultInstance()) {
            hNMEDAOCKOI_ = MusicInfoOuterClass.MusicInfo.newBuilder((MusicInfoOuterClass.MusicInfo) hNMEDAOCKOI_)
                .mergeFrom(value).buildPartial();
          } else {
            hNMEDAOCKOI_ = value;
          }
          onChanged();
        } else {
          if (hNMEDAOCKOICase_ == 9) {
            musicInfoBuilder_.mergeFrom(value);
          }
          musicInfoBuilder_.setMessage(value);
        }
        hNMEDAOCKOICase_ = 9;
        return this;
      }
      /**
       * <code>.MusicInfo music_info = 9;</code>
       */
      public Builder clearMusicInfo() {
        if (musicInfoBuilder_ == null) {
          if (hNMEDAOCKOICase_ == 9) {
            hNMEDAOCKOICase_ = 0;
            hNMEDAOCKOI_ = null;
            onChanged();
          }
        } else {
          if (hNMEDAOCKOICase_ == 9) {
            hNMEDAOCKOICase_ = 0;
            hNMEDAOCKOI_ = null;
          }
          musicInfoBuilder_.clear();
        }
        return this;
      }
      /**
       * <code>.MusicInfo music_info = 9;</code>
       */
      public MusicInfoOuterClass.MusicInfo.Builder getMusicInfoBuilder() {
        return getMusicInfoFieldBuilder().getBuilder();
      }
      /**
       * <code>.MusicInfo music_info = 9;</code>
       */
      @java.lang.Override
      public MusicInfoOuterClass.MusicInfoOrBuilder getMusicInfoOrBuilder() {
        if ((hNMEDAOCKOICase_ == 9) && (musicInfoBuilder_ != null)) {
          return musicInfoBuilder_.getMessageOrBuilder();
        } else {
          if (hNMEDAOCKOICase_ == 9) {
            return (MusicInfoOuterClass.MusicInfo) hNMEDAOCKOI_;
          }
          return MusicInfoOuterClass.MusicInfo.getDefaultInstance();
        }
      }
      /**
       * <code>.MusicInfo music_info = 9;</code>
       */
      private com.google.protobuf.SingleFieldBuilderV3<
          MusicInfoOuterClass.MusicInfo, MusicInfoOuterClass.MusicInfo.Builder, MusicInfoOuterClass.MusicInfoOrBuilder> 
          getMusicInfoFieldBuilder() {
        if (musicInfoBuilder_ == null) {
          if (!(hNMEDAOCKOICase_ == 9)) {
            hNMEDAOCKOI_ = MusicInfoOuterClass.MusicInfo.getDefaultInstance();
          }
          musicInfoBuilder_ = new com.google.protobuf.SingleFieldBuilderV3<
              MusicInfoOuterClass.MusicInfo, MusicInfoOuterClass.MusicInfo.Builder, MusicInfoOuterClass.MusicInfoOrBuilder>(
                  (MusicInfoOuterClass.MusicInfo) hNMEDAOCKOI_,
                  getParentForChildren(),
                  isClean());
          hNMEDAOCKOI_ = null;
        }
        hNMEDAOCKOICase_ = 9;
        onChanged();;
        return musicInfoBuilder_;
      }
      @java.lang.Override
      public final Builder setUnknownFields(
          final com.google.protobuf.UnknownFieldSet unknownFields) {
        return super.setUnknownFields(unknownFields);
      }

      @java.lang.Override
      public final Builder mergeUnknownFields(
          final com.google.protobuf.UnknownFieldSet unknownFields) {
        return super.mergeUnknownFields(unknownFields);
      }


      // @@protoc_insertion_point(builder_scope:uint32)
    }

    // @@protoc_insertion_point(class_scope:uint32)
    private static final Uint32.uint32 DEFAULT_INSTANCE;
    static {
      DEFAULT_INSTANCE = new Uint32.uint32();
    }

    public static Uint32.uint32 getDefaultInstance() {
      return DEFAULT_INSTANCE;
    }

    private static final com.google.protobuf.Parser<uint32>
        PARSER = new com.google.protobuf.AbstractParser<uint32>() {
      @java.lang.Override
      public uint32 parsePartialFrom(
          com.google.protobuf.CodedInputStream input,
          com.google.protobuf.ExtensionRegistryLite extensionRegistry)
          throws com.google.protobuf.InvalidProtocolBufferException {
        return new uint32(input, extensionRegistry);
      }
    };

    public static com.google.protobuf.Parser<uint32> parser() {
      return PARSER;
    }

    @java.lang.Override
    public com.google.protobuf.Parser<uint32> getParserForType() {
      return PARSER;
    }

    @java.lang.Override
    public Uint32.uint32 getDefaultInstanceForType() {
      return DEFAULT_INSTANCE;
    }

  }

  private static final com.google.protobuf.Descriptors.Descriptor
    internal_static_uint32_descriptor;
  private static final 
    com.google.protobuf.GeneratedMessageV3.FieldAccessorTable
      internal_static_uint32_fieldAccessorTable;

  public static com.google.protobuf.Descriptors.FileDescriptor
      getDescriptor() {
    return descriptor;
  }
  private static  com.google.protobuf.Descriptors.FileDescriptor
      descriptor;
  static {
    java.lang.String[] descriptorData = {
      "\n\014uint32.proto\032\021BalloonInfo.proto\032\016FallI" +
      "nfo.proto\032\017MusicInfo.proto\"\265\001\n\006uint32\022\022\n" +
      "\nminigameId\030\016 \001(\r\022\016\n\006isOpen\030\003 \001(\010\022\020\n\010ope" +
      "nTime\030\006 \001(\r\022$\n\014balloon_info\030\r \001(\0132\014.Ball" +
      "oonInfoH\000\022\036\n\tfall_info\030\013 \001(\0132\t.FallInfoH" +
      "\000\022 \n\nmusic_info\030\t \001(\0132\n.MusicInfoH\000B\r\n\013H" +
      "NMEDAOCKOIb\006proto3"
    };
    descriptor = com.google.protobuf.Descriptors.FileDescriptor
      .internalBuildGeneratedFileFrom(descriptorData,
        new com.google.protobuf.Descriptors.FileDescriptor[] {
          BalloonInfoOuterClass.getDescriptor(),
          FallInfoOuterClass.getDescriptor(),
          MusicInfoOuterClass.getDescriptor(),
        });
    internal_static_uint32_descriptor =
      getDescriptor().getMessageTypes().get(0);
    internal_static_uint32_fieldAccessorTable = new
      com.google.protobuf.GeneratedMessageV3.FieldAccessorTable(
        internal_static_uint32_descriptor,
        new java.lang.String[] { "MinigameId", "IsOpen", "OpenTime", "BalloonInfo", "FallInfo", "MusicInfo", "HNMEDAOCKOI", });
    BalloonInfoOuterClass.getDescriptor();
    FallInfoOuterClass.getDescriptor();
    MusicInfoOuterClass.getDescriptor();
  }

  // @@protoc_insertion_point(outer_class_scope)
}
